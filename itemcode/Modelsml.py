...
class MM_Model(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, weight_size, dropout_list, image_feats, text_feats,
                 user_init_embedding, item_attribute_dict, entity_num, entity2id):
        ...
        # 添加知识图谱实体嵌入层
        self.kg_entity_embeddings = nn.Embedding(entity_num, embedding_dim).cuda()
    ...
    def forward(self, ui_graph, iu_graph, image_ui_graph, image_iu_graph, text_ui_graph, text_iu_graph):
        ...
        # 融合知识图谱特征
        user_entity_mapping = [self.entity2id.get(f"User_{i}", 0) for i in range(self.n_users)]
        user_entity_ids = torch.tensor(user_entity_mapping).cuda()
        # 2. 物品实体ID映射（确保与知识图谱中的Item_xxx对应）
        item_entity_mapping = [self.entity2id.get(f"Item_{i}", 0) for i in range(self.n_items)]
        item_entity_ids = torch.tensor(item_entity_mapping).cuda()

        # 3. 融合用户/物品实体嵌入（区分权重）
        user_kg_embeds = self.kg_entity_embeddings(user_entity_ids)
        item_kg_embeds = self.kg_entity_embeddings(item_entity_ids)

        # 用户实体嵌入权重更高（可通过超参数调整）
        u_g_embeddings = u_g_embeddings + args.user_kg_rate * F.normalize(user_kg_embeds, p=2, dim=1)
        i_g_embeddings = i_g_embeddings + args.item_kg_rate * F.normalize(item_kg_embeds, p=2, dim=1)
